...
class DataProcessor:
    ...
    def plot_results(self, fq, r, Gr0):
        ...
        # Plotting S(Q)
        line3, = ax[1].plot(self.q, fq, label = "F(Q)" )
        ax[1].set_xlabel("Q ($\AA^{-1}$)")
        ax[1].set_ylabel("F(Q)")
        ax[1].legend()
        ax[1].title.set_text('Calculating F(Q)')
    ...
    def save_to_csv(self, data, file_path, name, separator, x, y, out='pdfgui'):
        ...
        if os.path.isfile(file_path):
            ...
            if out == 'discus':
                ...
                data.to_csv(f'{final_path}.csv', sep=separator, float_format="%.10f", index=False)
            else:
...
def read_discus_fit_file(path):
    #Function designed to read fitting data from DISCUS_SUITE
    df = pd.read_csv(path)
    df['#'] = df['#'].str.split(' ')
    df['#'] = df['#'].apply(remove_empty_strings)
    if len(pd.DataFrame(df['#'].tolist()).columns) == 4:
        df[['r', 'gr', 'dr', 'dgr']] = pd.DataFrame(df['#'].tolist(), index=df.index)
    else:
        df[['r', 'gr']] = pd.DataFrame(df['#'].tolist(), index=df.index)
